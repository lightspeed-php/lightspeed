...
abstract class BaseRunner implements RunnerInterface
{
    ...
    private function load(SuiteLoader $loader): void
    {
        ...
        // inject lightspeed logic to split tests
        $provider = ProviderFactory::resolveProvider();
        if ($provider instanceof Provider) {
            $this->pending = $loader->getTestMethods();
            $this->sortPending();
            $sliceLength = ceil(count($this->pending) / $provider->nodeCount());
            $offset = $sliceLength * $provider->nodeIndex();
            $this->pending = array_slice($this->pending, $offset, $sliceLength);
        } else {
